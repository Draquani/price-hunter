...
export default function PriceResults({ result }: Props) {
  ...
  const cheapest = availableResults.length
    ? availableResults.reduce((a, b) => parseFloat(a.price.replace(/[$,]/g, "")) < parseFloat(b.price.replace(/[$,]/g, "")) ? a : b)
    : null;
    ...
    <div className="mt-3 rounded-2xl overflow-hidden border border-white/20 shadow-2xl bg-white/10 backdrop-blur-sm">
      <div className="flex items-center gap-4 p-4 bg-gradient-to-r from-violet-600/30 to-fuchsia-600/30">
        ...
        <div>
          ...
          {cheapest && <p className="text-emerald-300 text-sm font-semibold mt-1">🏆 Best price: {cheapest.price} at {cheapest.store}</p>}
        </div>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-white/10 border-b border-white/10">
              {["Store", "Price", "Link", "Note"].map((h) => (
                <th key={h} className="text-left px-4 py-2.5 text-white/60 font-semibold uppercase text-xs tracking-wider">{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {result.results.map((row, i) => {
              ...
              return (
                <tr key={i} className={`border-b border-white/5 transition-colors ${isBest ? "bg-emerald-500/10" : "hover:bg-white/5"}`}>
                  <td className="px-4 py-3 text-white font-medium"><span className="flex items-center gap-2">{isBest && <span className="text-emerald-400">★</span>}{row.store}</span></td>
                  <td className="px-4 py-3"><span className={`font-bold text-base ${row.available ? (isBest ? "text-emerald-300" : "text-white") : "text-white/30"}`}>{row.price}</span></td>
                  <td className="px-4 py-3">{row.url && row.available ? <a href={row.url} target="_blank" rel="noopener noreferrer" className="text-fuchsia-300 hover:text-fuchsia-200 underline underline-offset-2 text-xs truncate max-w-[160px] block">View product →</a> : <span className="text-white/30 text-xs">—</span>}</td>
